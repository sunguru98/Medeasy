...
export const AutoSelect = ({ products, isNormalItem, onClick }) => {
  ...
  const handleRequest = () => {
    setAutoSelectState(false)
    const tabletRequest = window.prompt(
      'Please type in the product you intended to buy along with the respective dosage and quantity separated by a comma.'
    )
		Axios.post('/api/queries/tablet', { tabletRequest })
		alert('Thank you for the request. If you want to have a detailed/customised order means, React out to contact@medeasyonline.com')
  }
    ...
    <AutoSelectContainer>
      ...
      {products && autoSelectState && searchText.length > 0 ? (
        <SearchResultList>
          ...
          {!isNormalItem ? (
            <SearchResultListItem onClick={handleRequest}>
              <SearchResultDetails style={{ width: '100%' }}>
                <p
                  style={{
                    textAlign: 'center',
                    padding: '1rem',
