...
const AccountPhase = ({ signIn, signUp, alertUser, setCheckoutRole, setStepProgress, user, history, orderId }) => {
	...
	const registerUser = event => {
		...
		else if (password.length < 8) alertUser('Password length must be atleast 8 characters', 'danger')
		else signUp({ name: `${fName} ${mName.length > 0 ? mName : ''}${mName ? ' ': ''}${lName}`, email, password }, 'passon')
	}
			...
			{mode === 'guest' && (
				<div className="AccountPhase__guest">
					<p className="AccountPhase__guest--info">
						<span
							onClick={() => {setAuthMode('login'); setMode('user') }}
							style={{ color: '#7AC7B8', cursor: 'pointer' }}
						>
							Login
						</span>{' '}
						or{' '}
						<span
							onClick={() => {setAuthMode('register'); setMode('user') }}
							style={{ color: '#7AC7B8', cursor: 'pointer' }}
						>
							Register
						</span>{' '}
					...
					<CustomButton onClick={() => { setCheckoutRole('guest'); history.push('/checkout/address') }}>Continue as Guest</CustomButton>
				</div>
			)}
		</div>
	)
