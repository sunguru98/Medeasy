...
module.exports = {
	...
	createProduct: async (req, res) => {
		...
		if (req.files.length < 3)
			return res
				.status(400)
				.send({ statusCode: 400, message: 'Minimum 3 images required' })
		...
		try {
			...
			if (await Product.findOne({ name: req.body.name }))
				return res
					.status(400)
					.send({ statusCode: 400, message: 'Product already exists' })

			const productObj = prepareProductObject(req.body)
			if (!productObj)
				return res
					.status(400)
					.send({ statusCode: 400, message: 'Bad request on Price upload' })
			...
			if (err.name === 'CastError')
				return res
					.status(400)
					.send({ statusCode: 400, message: 'Invalid Product Id' })
			res.status(500).send({ statusCode: 500, message: 'Server Error' })
		}
	},

	// Make products featured
	async makeProductsFeatured(req, res) {
		const { productIds } = req.body
		if (productIds.length < 5)
			return res
				.status(400)
				.send({ statusCode: 400, message: 'Minimum 5 products required' })
		try {
			...
			Promise.all(
				productIds.map(
					pId =>
						new Promise(resolve => {
							Product.findByIdAndUpdate(pId, { $set: { featured: true } }).then(
				...
				.then(() =>
					res
						.status(202)
						.send({ statusCode: 202, message: 'Products Updated successfully' })
				)
	...
	fetchAllProductsByCategory: async (req, res) => {
		...
		if (!categoryId)
			return res
				.status(400)
				.send({ statusCode: 400, message: 'Category Id not found' })
		try {
	...
	fetchProductById: async (req, res) => {
		...
		if (!productId)
			return res
				.status(400)
				.send({ statusCode: 400, message: 'Product Id not found' })
		try {
	...
	updateProductById: async (req, res) => {
		...
		try {
			...
			let product = await Product.findOneAndUpdate(
				{ admin: req.user._id, _id: productId },
				{ $set: prepareProductObject(req.body) },
				{ new: true }
			)
			...
			fs.readdir(pathName2, (err, files) => {
				files.forEach(file => {
					if (!originalFileNames.includes(file)) {
						fs.unlink(
							path.join(__dirname, `../uploads/${product.name}/${file}`),
							() => {}
						)
					}
				})
